...
async def try_cerebras_then_gemini(cerebras_callable, gemini_callable):
    try:
        return await cerebras_callable()
    except Exception as e:
        logger.warning(f"Cerebras failed: {e}. Falling back to Gemini.")
        return await gemini_callable()
...
async def generate_button_suggestions(system_prompt: str, timeout: int = 10) -> str:
    ...
    return await try_cerebras_then_gemini(
        lambda: cerebras_call("", system_prompt, CEREBRAS_MODEL_BUTTON, timeout),
        lambda: gemini_call("Generate buttons", system_prompt)
    )
...
async def generate_block_content(prompt: str, system_prompt: str) -> str:
    ...
    return await try_cerebras_then_gemini(
        lambda: cerebras_call(prompt, system_prompt, CEREBRAS_MODEL_BLOCK),
        lambda: gemini_call(prompt, system_prompt)
    )
